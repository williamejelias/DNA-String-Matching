...
alignments_list = []
def allAlignments(n, m, seq1, seq2):
    if len(seq1) == 0 or len(seq2) == 0:                    #if seq1 or seq2 are empty then fill up the remaining
        ...
        n = "-" + n
        m = "-" + m
        allAlignments(n_new, m_new, seq1[:-1], seq2[:-1])   #case where (n, m) added into alignment
        allAlignments(n, m_new, seq1, seq2[:-1])            #case of (blank, m) added into alignment
        allAlignments(n_new, m, seq1[:-1], seq2)            #case of (n, blank) added into alignment        
