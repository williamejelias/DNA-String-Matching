    ...
    for row in range(1, m):
        for column in range(1, n):
            ...
            if row == m-1 and column == n-1:
                score_matrix[row][column] = max(match, gap_seq1 - gap_score, gap_seq2 - gap_score)  #no gap penalty moving from bottom right cell
            elif row == m-1:
                score_matrix[row][column] = max(match, gap_seq1 , gap_seq2- gap_score)  #no gap penalty moving from final row
            elif column == n-1:
                score_matrix[row][column] = max(match, gap_seq1- gap_score, gap_seq2 )  #no gap penalty moving from final column
            else:
                score_matrix[row][column] = max(match, gap_seq1, gap_seq2)      #value in matrix takes the maximum value of the score for each possibility up to that point
            global best_score
            best_score = score_matrix[m-1][n-1]                                 #update best_score with the value in the bottom right corner of the matrix

    for row in range(1, m):
        for column in range(1, n):
            if row == m-1 and column == n-1:
                if score_matrix[row][column] == (score_matrix[row-1][column-1] + score(column-1, row-1)):
                    traceback_matrix[row][column] = "D"         #insert "D" into the traceback matrix if the max score came from the entry left-diagonally above 
                elif score_matrix[row][column] == score_matrix[row-1][column]:
                    traceback_matrix[row][column] = "U"         #insert "U" into the traceback matrix if the max score came from the entry above / no gap penalty
                elif score_matrix[row][column] == score_matrix[row][column-1]:
                    traceback_matrix[row][column] = "L"         #insert "L" into the traceback matrix if the max score came from the entry left  / no gap penalty
            elif row == m-1:
                if score_matrix[row][column] == (score_matrix[row-1][column-1] + score(column-1, row-1)):
                    traceback_matrix[row][column] = "D"         #insert "D" into the traceback matrix if the max score came from the entry left-diagonally above
                elif score_matrix[row][column] == (score_matrix[row-1][column] + gap_score):
                    traceback_matrix[row][column] = "U"         #insert "U" into the traceback matrix if the max score came from the entry above
                elif score_matrix[row][column] == score_matrix[row][column-1]:
                    traceback_matrix[row][column] = "L"         #insert "L" into the traceback matrix if the max score came from the entry left  / no gap penalty
            elif column == n-1:
                if score_matrix[row][column] == (score_matrix[row-1][column-1] + score(column-1, row-1)):
                    ...
                    traceback_matrix[row][column] = "L"         #insert "L" into the traceback matrix if the max score came from the entry left  
            else:
                if score_matrix[row][column] == (score_matrix[row-1][column-1] + score(column-1, row-1)):
                    traceback_matrix[row][column] = "D"         #insert "D" into the traceback matrix if the max score came from the entry left-diagonally above
                elif score_matrix[row][column] == (score_matrix[row-1][column] + gap_score):
...
# Call the function to create the list of alignments
# Call the function to score each of the alignments.  
# To work with the printing functions below your list of alignments should be called alignments_list. The best alignment should be called best_alignment and its score should be called best_score. 

dynamicAlignment(seq1, seq2)
